...
import org.clothocad.tool.trumpet.results.ButtonTabComponent;
import org.clothocad.tool.trumpet.help.HTMLViewer;
...
import org.clothocad.tool.trumpet.trees.KeyTree;
import org.clothocad.tool.trumpet.trees.InvEdge;
import org.clothocad.tool.trumpet.trees.MyMousePlugin;
...
import org.clothocad.tool.trumpet.draganddrop.ObjBaseDropTarget;
import org.clothocad.tool.trumpet.draganddrop.ObjBaseTransferHandler;
 ...
 *
 */
public class TrumpetGUI extends javax.swing.JFrame implements ObjBaseDropTarget {
    ...
    public TrumpetGUI() {
        ...
        jPartsList.setTransferHandler(new ObjBaseTransferHandler(this));
...
//\\//\\//\\//\\//\\//\\//\\//\\ HARD CODED //\\//\\//\\//\\//\\//\\//\\//\\//\\
     ...
     */
    public void addInvertases(int invs)
    {
...
//                    _mousePlugin,form,jKeyTableMenuItem.isSelected(), jKeyTreesMenuItem.isSelected());
    ...
    private void initComponents() {
        ...
        jCloseMenuItem.setText(org.openide.util.NbBundle.getMessage(TrumpetGUI.class, "TrumpetGUI.jCloseMenuItem.text")); // NOI18N
...
//            _controller.setNumberOfParts(n);
    ...
    private void jFormatComboBoxActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jFormatComboBoxActionPerformed
        ...
        if ((!(_currentFormat.toString().equals(nextForm.toString()))) && (jPartsList.getModel().getSize()!=0))
        {
    ...
    /**
     * Displays the current version of Trumpet.
     * @param evt
     */
    ...
    /**
     * Displays the user manual for Trumpet in a dialog box.
     * @param evt
     */
    private void jInstructionsMenuItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jInstructionsMenuItemActionPerformed
        ...
        String test = this.getClass().getResource("/org/clothocad/tool/trumpet/help/InstructionSet.html").toString();

        if (test != null) {
            try {
                editorPane.setPage(test);
            } catch (IOException e) {
                System.err.println("Attempted to read a bad URL: " + test);
            }
        } else {
        ...
        HTMLViewer instructions = new HTMLViewer("Trumpet User Manual",editorPane);

        instructions.setMinimumSize(new Dimension(540,800));
    ...
    public void handleDropedObject(ObjBase o) {
         ...
         if (o.getType().equals(ObjType.PART))
        {
            ...
            if (toAdd.getFormat().toString().equals(this.jFormatComboBox.getSelectedItem().toString()))
            {
                _controller.appendPart(jPartsList, toAdd.toString());
                updatePermutationsList();
                _controller.clearList(jDesiredPermList);
            }
            else
            {
                //custom title, warning icon
                JOptionPane.showMessageDialog(new JFrame(),
                "This part does not agree with your\n"+
