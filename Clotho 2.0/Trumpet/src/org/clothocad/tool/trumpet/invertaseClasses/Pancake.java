 ...
 *
 */
package org.clothocad.tool.trumpet.invertaseClasses;
...
public class Pancake extends InvertSim {
    ...
    public void createDesign(int n)
    {
        ...
        for (int i=0; i<n; i++)
        {
            ...
            if (combos)
            {
                // n=1 is a special case, without this line, n=1 will produce
                // I(-1) P1 I(-1)' - we want I1 P1 I1'
                if (n==1)
                    design = design.concat("I"+1+" ");
                else
    ...
    public String generateKey(String target)
    {
        ...
        // If the string target is simply P1 or P1', the key generator algorithm
        // will spit out the wrong key.
        // Due to this, I have included the following few lines of code.
        if ((targetStringPerm.length==1) && (targetStringPerm[0].equals("P1'")))
            return "1";
        if ((targetStringPerm.length==1) && (targetStringPerm[0].equals("P1")))
            return "";
