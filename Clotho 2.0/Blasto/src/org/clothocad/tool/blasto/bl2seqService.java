...
import org.biojava3.ws.alignment.qblast.BlastJob;
import org.biojava3.ws.alignment.qblast.MapToStringTransformer;

/*
 * slight rewrite of the NCBIQBlastService class from the Biojava package to accomadate bl2seq
 */
public class bl2seqService {
...
public static final long WAIT_INCREMENT = 3000;
	...
	private static final String SERVICE_URL = "http://www.ncbi.nlm.nih.gov/blast/BlastAlign.cgi";
	private static final String DEFAULT_EMAIL = "anonymous@clotho.org";
	private static final String DEFAULT_TOOL = "clotho";
	 ...
	 */
	public String sendAlignmentRequest(String query, String subject) throws Exception {
                
		String putCmd = "CMD=Put&PROGRAM=blastn&BLAST_SPEC=blast2seq&";
                putCmd = putCmd.concat("QUERY="+query+"&");
                putCmd = putCmd.concat("SUBJECTS="+subject);
		OutputStreamWriter writer = null;
		BufferedReader reader = null;
		try {
			URLConnection serviceConnection = setQBlastServiceProperties(serviceUrl.openConnection());
	...
	public boolean isReady(String id, long present) throws Exception {
		...
		if (job != null) {
                        job.setExpectedExecutionTime(WAIT_INCREMENT);
