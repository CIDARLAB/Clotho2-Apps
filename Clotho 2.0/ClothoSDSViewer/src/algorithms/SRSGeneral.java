...
public class SRSGeneral extends SRSAlgorithmCore{
    ...
    protected SRSGraph combineGraphsModEff(ArrayList<SRSGraph> graphs, HashSet<String> recommended, HashMap<String, Integer> sharing, HashMap<Integer, HashMap<String, Double>> modularityHash, HashMap<Integer, Double> efficiencies) {
        ...
        for (int i = 0; i < graphs.size(); i++) {
            modularity = graphs.get(i).getModularity() + modularity;
        ...
        combineGraphsME.setModularity(modularity);
    ...
    protected SRSGraph combineGraphsStageStep(ArrayList<SRSGraph> graphs) {
        ...
        for (int i = 0; i < graphs.size(); i++) {
            ...
            //If it's the first graph, assign that root's left-most overhang to this graph's left overhang, if it's last, assign the right
            if (i == 0) {
                newRoot.setLOverhang(currentNeighbor.getLOverhang());
            } else if (i == (graphs.size() - 1)) {
                newRoot.setROverhang(currentNeighbor.getROverhang());
            }
        }
